...
export function analyzeSEO(
  ...
  const readabilityScore = calculateReadabilityScore(content, words);
  const keywordDensity = targetKeyword
    ? calculateKeywordDensity(content, targetKeyword, wordCount)
    : 0;
  ...
  if (targetKeyword) {
    if (keywordDensity === 0) {
      ...
      suggestions.push(
        `Increase usage of target keyword "${targetKeyword}". Current density: ${keywordDensity.toFixed(2)}%`,
      );
    } else if (keywordDensity > 3) {
      issues.push(
        `Keyword density too high (${keywordDensity.toFixed(2)}%). Risk of keyword stuffing.`,
      );
    } else {
      strengths.push(
        `Good keyword density for "${targetKeyword}": ${keywordDensity.toFixed(2)}%`,
      );
    }
  }
...
function calculateReadabilityScore(text: string, wordsArray: string[]): number {
  // Simplified Flesch Reading Ease score
  const sentences = text
    .split(/[.!?]+/)
    .filter((s) => s.trim().length > 0).length;
  const wordsCount = wordsArray.length;
  const syllables = estimateSyllables(wordsArray);

  if (sentences === 0 || wordsCount === 0) return 0;

  const avgSentenceLength = wordsCount / sentences;
  const avgSyllablesPerWord = syllables / wordsCount;
...
function calculateKeywordDensity(
  ...
  return (keywordCount / wordsCount) * 100;
}
...
function calculateSEOScore(params: {
  ...
  if (params.readabilityScore >= 60 && params.readabilityScore <= 80)
    score += 25;
  else if (params.readabilityScore >= 50 && params.readabilityScore <= 90)
    score += 20;
  ...
  else if (params.keywordDensity > 0 && params.keywordDensity < 0.5)
    score += 10;
